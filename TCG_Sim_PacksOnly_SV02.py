...
# Read the file
data = pd.read_csv("SV02_Compiled_Simple_RH.csv",
                   header=0,
...
for i in range(1, 36001):
    # Select Series of Common Cards (Slots 1-4). Add Pack #. Merge to Sim_Results.
    ...
    # Select Series of Uncommon Cards (Slots 5-7). Add Pack #. Merge to Sim_Results.
    ...
    # Select Reverse Holo (Slot 8). Add Pack #. Merge to Sim_Results.
    Reverse_Holo = df.loc[:, ['Card_Name', 'Number', 'Rarity', 'Market_Price', 'Adjusted_Price']].sample(
        1, weights=df.Slot_8)
    Reverse_Holo['Pk_Num'] = i
    Sim_Results = Sim_Results.merge(Reverse_Holo, how='outer')
    # Select Illustration Rare/Reverse Holo (Slot 9). Add Pack #. Merge to Sim_Results.
...
Sim_Results.to_csv('Pack_Sim_Results_SV02.csv', encoding='utf-8', index=False)
